...
export function PromptCopyButton({
  ...
  const text = state === "done" ? doneLabel : state === "error" ? "复制失败，请重试" : state === "loading" ? "正在复制…" : label;
  ...
  return (
    ...
    >
      <Icon aria-hidden className={cn("size-3.5", state === "loading" && "motion-safe:animate-spin")} />
